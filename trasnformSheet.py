import pandas as pd
from tkinter import Tk, Canvas, Entry, Button, filedialog, messagebox, PhotoImage
...
def select_training_file():
    ...
    training_file = filedialog.askopenfilename(
        title="Select Training File", filetypes=[("Excel files", "*.xlsx;*.xls")]
    )
    ...
    if training_file:
        ...
        display_name = (
            str_file_name if len(str_file_name) <= 60 else str_file_name[-60:]
        )
...
def process_files():
    ...
    try:
        ...
        df_email = pd.read_csv(
            "view_base_funcionarios.csv", encoding="windows-1252", delimiter=";"
        )

        df_training["Matricula_temp"] = (
            df_training["Matricula"].astype(str).str.zfill(8)
        )
        df_email["Matrícula do Funcionário"] = (
            df_email["Matrícula do Funcionário"].astype(str).str.zfill(8)
        )

        df_merged = pd.merge(
            df_training,
            df_email[
                ["Matrícula do Funcionário", "Email Gestor", "E-mail do Funcionário"]
            ],
            left_on="Matricula_temp",
            right_on="Matrícula do Funcionário",
            how="left",
        )
        df_merged.drop(
            columns=["Matricula_temp", "Matrícula do Funcionário"], inplace=True
        )
...
window.title("Formtar os dados da Planilha de Treinamento")
